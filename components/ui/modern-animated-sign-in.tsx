...
// Image component removed - using regular img tags for Vite compatibility
import {
  motion,
  useAnimation,
  useInView,
...
const Ripple = memo(function Ripple({
  ...
  return (
    <section
      className={`${className.includes('!max-w-full') ? 'max-w-full' : 'max-w-[50%]'} absolute inset-0 flex items-center justify-center
        ${className.includes('!max-w-full') ? '' : 'dark:bg-white/5 bg-neutral-50'}
        ${className.includes('!max-w-full') ? '' : '[mask-image:linear-gradient(to_bottom,black,transparent)]'}
        ${className.includes('!max-w-full') ? '' : 'dark:[mask-image:linear-gradient(to_bottom,white,transparent)]'} ${className}`}
    >
      ...
      <form onSubmit={handleSubmit}>
        <section
          className={cn(
            ...
            fieldPerRow === 3 && "md:grid-cols-3",
            fieldPerRow === 4 && "md:grid-cols-4"
          )}
        >
          {fields.map((field) => (
