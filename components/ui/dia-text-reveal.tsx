...
function buildGradient(pos: number, colors: string[], textColor: string) {
  ...
  if (bandEnd < 100) {
    const trailStart = Math.max(bandEnd, 0).toFixed(2)
    parts.push(`${textColor} ${trailStart}%`, `${textColor} 100%`)
  }
...
export function DiaTextReveal({
  ...
  return (
    <motion.span
      ref={spanRef}
      className={cn(
        "max-w-full text-inherit",
        isMulti
          ? "inline-block overflow-hidden whitespace-nowrap align-middle text-center leading-none"
          : "block w-full whitespace-normal text-balance leading-tight",
        className
      )}
      style={{
        color: "transparent",
