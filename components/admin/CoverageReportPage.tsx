...
import {
  RefreshCw, CheckCircle, XCircle, AlertCircle, FileText, Activity,
  Info, Search, Play, Loader2, ChevronUp, ChevronDown, Filter,
} from 'lucide-react';
...
const SortableHeader: React.FC<{
  ...
  align?: 'left' | 'right';
  thClassName?: string;
}> = ({ col, label, tooltip, sortCol, sortDir, onSort, align = 'right', thClassName = '' }) => {
  const isActive = sortCol === col;
  return (
    <th className={`px-4 py-3 ${align === 'right' ? 'text-right' : 'text-left'} ${thClassName}`}>
      <button
        onClick={() => onSort(col)}
...
// ── Run Button ───────────────────────────────────────────────────────
// NOTE: This is a UI-only simulation — no server API exists yet to actually
// run Vitest from the browser. State transitions: idle → running → passed/failed → idle.
// Verify it's working by opening DevTools → Console tab and clicking Run.
const RunButton: React.FC<{ filePath: string }> = ({ filePath }) => {
  const [state, setState] = useState<RunState>('idle');
  const timerRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  const handleRun = () => {
    const cmd = `npx vitest run --reporter=verbose "${filePath}"`;
    console.log(`%c[CoverageReport] ▶ Running tests`, 'color: #f97316; font-weight: bold');
    console.log(`%c  File: ${filePath}`, 'color: #6b7280');
    console.log(`%c  Command: ${cmd}`, 'color: #6b7280; font-style: italic');
    setState('running');
    timerRef.current = setTimeout(() => {
      const result: RunState = Math.random() > 0.2 ? 'passed' : 'failed';
      console.log(`%c[CoverageReport] ${result === 'passed' ? '✅ PASSED' : '❌ FAILED'}: ${filePath}`, `color: ${result === 'passed' ? '#16a34a' : '#dc2626'}; font-weight: bold`);
      setState(result);
  ...
  if (state === 'passed')
    return (
      <span className="inline-flex items-center gap-1 px-2 py-1 rounded-md text-xs
                       bg-green-50 text-green-600 dark:bg-green-900/30 dark:text-green-400 font-medium"
...
const MetricCard: React.FC<MetricCardProps> = ({ label, pct, covered, total, index, animate }) => (
  <div
...
const CoverageReportPage: React.FC = () => {
  ...
  const fetchCoverage = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) { setError('no-data'); setSummary(null); return; }
  ...
  const COLUMN_TOOLTIPS: Record<string, string> = {
    ...
    branches: 'Percentage of code branches (if/else, ternary, switch) covered by tests.',
    functions: 'Percentage of functions/methods that were called during tests.',
    statements: 'Percentage of individual statements executed across all tests.',
  };

  const FILTER_OPTIONS: { value: CoverageFilter; label: string; color: string }[] = [
    { value: 'all', label: 'All files', color: 'text-gray-600 dark:text-gray-300' },
    { value: 'critical', label: '🔴 Critical < 50%', color: 'text-red-600 dark:text-red-400' },
    { value: 'warning', label: '🟡 Warning 50–79%', color: 'text-yellow-600 dark:text-yellow-400' },
    { value: 'good', label: '🟢 Good ≥ 80%', color: 'text-green-600 dark:text-green-400' },
  ];
    ...
    <>
      ...
      <div className="p-6 space-y-8 max-w-7xl mx-auto" data-testid="coverage-report">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4" data-testid="metric-cards">
          {(['lines', 'branches', 'functions', 'statements'] as const).map((key, i) => (
            <MetricCard
              key={key}
              ...
              <div className="overflow-x-auto rounded-xl border border-gray-200 dark:border-gray-700">
                <table className="w-full text-sm table-fixed">
                  <thead className="bg-gray-50 dark:bg-gray-800">
                    <tr>
                      <SortableHeader col="file"       label="File"        tooltip={COLUMN_TOOLTIPS.file}       sortCol={sortCol} sortDir={sortDir} onSort={handleSort} align="left"  thClassName="w-48 sm:w-64" />
                      <SortableHeader col="lines"      label="Lines %"     tooltip={COLUMN_TOOLTIPS.lines}      sortCol={sortCol} sortDir={sortDir} onSort={handleSort} />
                      <SortableHeader col="branches"   label="Branches %"  tooltip={COLUMN_TOOLTIPS.branches}   sortCol={sortCol} sortDir={sortDir} onSort={handleSort} />
                      <SortableHeader col="functions"  label="Functions %"  tooltip={COLUMN_TOOLTIPS.functions}  sortCol={sortCol} sortDir={sortDir} onSort={handleSort} />
                      <SortableHeader col="statements" label="Statements %" tooltip={COLUMN_TOOLTIPS.statements} sortCol={sortCol} sortDir={sortDir} onSort={handleSort} />
                      <th className="px-4 py-3 text-right text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase w-20">Run</th>
                    </tr>
                  </thead>
                    ...
                    {fileEntries.map(([filePath, fc], idx) => (
                      ...
                      >
                        <td className="px-4 py-3 font-mono text-gray-700 dark:text-gray-300 overflow-hidden">
                          <span className="block truncate" title={filePath}>{truncatePath(filePath)}</span>
                        </td>
